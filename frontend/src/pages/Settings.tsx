...
function Settings() {
  ...
  const userDefaultObj: {login?: string, email?: string, password?: string, newPassword?: string} = {
    login: user?.login,
    email: user?.email,
    password: '',
    newPassword: ''
  ...
  const [userData, setUserData] = useState<{login?: string, email?: string, password?: string, newPassword?: string}>(userDefaultObj)

  useEffect(() => {
    setUserData(state => ({...state, email: user?.email, login: user?.login}))
    console.log(user)
  }, [user])
  
  const updateEmail = async () => {
    const res = await dashboardStore.updateUser({login: userData?.login, email: userData.email})
    setUserData({login: res?.login, email: res?.email})
    setUser((state) => ({...state, id: res!.id, login: res?.login as string, email: res?.email as string}))
    console.log(res)
  }
  ...
  return (
    <div className="flex-1 space-y-8 h-full w-full mx-auto">
      {/* Заголовок страницы */}
      <div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <Button onClick={updateEmail} disabled={!userData.login?.length || !userData.email?.length} variant="outline" className="w-fit">Оновити профіль</Button>
        </div>
      </div>
