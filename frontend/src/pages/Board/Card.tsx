...
export default function Card({card, columnId}: {card: TCard, columnId: number}) {
  ...
  // Здесь карточка руинит перетаскивание колонки
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({id: card?.id as number, data: {type: 'card', columnId}})
